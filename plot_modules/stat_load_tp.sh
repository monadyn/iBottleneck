...
backup_stat_files(){
	cur_dir=`pwd`
	pushd $one_type_srv_test_root
	#/test_data/haboob_cal1/t1/haboob_cal1_1600
	#mkdir ${one_conc_test_root}
	...
	echo
	echo "copy stat files -> $one_conc_test_root"
	cp inout.txt ${one_conc_test_root}
	cp multiplicity.txt  ${one_conc_test_root}
	cp responsetime.txt  ${one_conc_test_root}
	cp cdata.txt  ${one_conc_test_root}

	if ( is_file_exists "inout_longReq.txt" )
	then	
		cp inout_longReq.txt   ${one_conc_test_root}
	else
		echo 
	fi
	
	if ( is_file_exists "responsetime_longReq.txt" )
	then	
		cp responsetime_longReq.txt  ${one_conc_test_root}
	else
		echo
	fi
	
	cp timerange.txt  ${one_conc_test_root}
	cp tablefilename ${one_conc_test_root}
	cp presdata.csv  ${one_conc_test_root}
